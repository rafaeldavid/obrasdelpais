...
1. **Live preview lives at https://steady-glacier-drz3.here.now/** — re-deploy with the [`here-now`](https://here.now) skill (`~/.claude/skills/here-now/scripts/publish.sh . --slug steady-glacier-drz3 --client claude-code`). State is persisted in `.herenow/state.json` and the API key in `~/.herenow/credentials`.
...
| 5b | **Custom domain `obrasdelpais.info` → here.now preview** | Domain is registered with here.now (status: `pending`). DNS not yet pointed at here.now. | At the domain registrar, add: `A @ 162.159.142.245`, `A @ 172.66.2.241`, `CNAME www fallback.here.now`. (Cloudflare users: turn proxy off / gray cloud.) Once propagated, run `scripts/link-domain.sh` — it polls status and creates the link to slug `steady-glacier-drz3` once the domain is `active`. |
